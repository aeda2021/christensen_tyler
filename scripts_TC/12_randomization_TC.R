...
###Distributions are similar in overall shape, but there are differences.  Both hists have a large number of ca. 2015 observations
###and many evenly distributed observations between ca. 1975 and 2005.
...
###if we sample without replacement and our resample size is 50, then the distribution will be exactly the same 
###as the original (had to think about that for a minute...)
...
pennies_resample_means <- aggregate(x = pennies_resample$year,
          by = list(pennies_resample$replicate),
          FUN = mean)

hist(pennies_resample_means$x)
...
###If the original sample was actually representative of the population, that representation should include 
###both high and low values.  Because our bootstrap resampling can select those values multiple times, some replicates
###should have low and high means.
...
ggplot(data = pennies_resample_means, aes(x=x)) +
...
pennies_resample_medians <- aggregate(x = pennies_resample$year,
                                    by = list(pennies_resample$replicate),
                                    FUN = median)

# make the histogram, and add quantiles that include 95% of the estimated medians

hist(pennies_resample_medians$x)
...
setwd("C:/Users/piedm/Dropbox/R/GitandR/christensen_tyler/raw-data_TC")

bee_prefs <- read.csv("bee_prefs.csv")
# Q: are these data tidy?  should they be?

###They are NOT tidy.  I don't know whether they should or shouldn't be, but I'm growing accustomed to tidy format data, so that would be my instinct.
...
###Because permutation is resampling without replacement; if replacement is false, then it's permutation
...
###Yes and yes; sums should be the same because existing values are just shuffled around
...
###No and no; because values were permuted within columns, values will be different in each row each permutation
...
###it first takes a random sample of size 1 from observations of number of visits; it then takes a random sample of Monarda pollen counts of a sample size equal to the number of visits 
###the last line sums the number of pollen grains 

# Q: why did we use size=visit_draw?

###I THINK this was because we are interested in the amount of pollination going on, which is a function of pollen grains per visit AND the number of visits.  If a bee visits 6 times, you want to know how much 
###pollen might be deposited in 6 visits
...
hist(rgamma(173, mean_vis), breaks = 50)
...
###gamma wins with the lowest AIC.

# Q: write code below to choose a distribution for the pollen data

grains_pois <- fitdist(pollen$consp_grains, "pois")
summary(grains_pois)

grains_gamma <- fitdist(pollen$consp_grains, "gamma")
summary(grains_gamma)

grains_nbinom <- fitdist(pollen$consp_grains, "nbinom")
summary(grains_nbinom)
...
hist(pollen$consp_grains, breaks = 50)
mean_pol <- mean(pollen$consp_grains)
mean_pol
hist(rpois(173, mean_pol), breaks = 50)

sd_pol <- sd(pollen$consp_grains)
sd_pol
shape1 <- (mean_pol/sd_pol)^2
shape1
scale1 <- sd_pol^2 / mean_pol
scale1
...
bee_mass <- read_csv("bee_mass.csv")
glimpse(bee_mass)
...
###It returns a table with the mean for each of the 100 bootstrap replicates of 'dry_mass_mg'.  
...
mass_mean_CI <- mass_means %>%
    summarize(l = quantile(stat, prob = 0.025), u = quantile(stat, prob = 0.975))
...
###'stat' contained the difference in means between queen and workers in each permnuted replicate

# Q: how will you use these numbers? what are we trying to test here?

###we will use these means to generate a null distribution of means and its confidence interval 
...
###If I'm understanding this correctly, the null permutation permutes the data within the dry_mass column
###without regard to identity, so the difference in means should dance around 0 (any differences that were there
###were dissolved by permuting within the column)
...
###I think there will be a significant difference between the null permutation-generated data and the observed data
...
null_dist %>%
    visualize(binwidth = 50) +
    shade_p_value(obs_stat = 5, direction = "two-sided")
