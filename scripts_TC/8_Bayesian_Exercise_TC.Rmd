...
setwd("C:/Users/piedm/Dropbox/R/GitandR/2021_master/scripts/")
...
source('C:/Users/piedm/Dropbox/R/GitandR/2021_master/scripts/8_bayesFunctions.R')
```

# JAGS, an introduction
Before we get into how to code and run our models, let's briefly consider the algorithm and software we are running. The software is JAGS (Just Another Gibbs Sampler), which compiles a number of algorithms to estimate Bayesian posteriors using Markov Chain Monte Carlo (MCMC) sampling.
...
*Answer*: [b0, b1, e | y] ∝ [y | b0, b1, e]*[b0]*[b1]*[e]

[b0, b1, e | yi] ∝ norm(yi | bo, b1, e) * norm(0,100) * norm(0,100) * gamma(1,0.01)

"The posterior probability of the parameters b0, b1, and e given the data y is proportional to the probability of the data y given the parameters b0, b1, and e multiplied by the (prior) marginal probability distributions of b0, b1, and e."
...
The marginal probability is the estimated probability distribution of an event (or parameter values) when it is NOT conditioned on other events (or parameter values).  Marginal posterior probability distributions are  how we assess the probability of a model given the observed data.  
...
lmm.data <- read.csv("C:/Users/piedm/Dropbox/R/GitandR/2021_master/raw-data/8_mixedEffectDat.csv")
```
...
*Answer* [b0_j, b1, σ2, μ, s2 | y_ij] ∝ [y_ij | b0_j, b1, σ2]* [b0j | μ, s2] * [μ] * [s2] * [σ2]  

[b0_j, b1, σ2, μ, s2 | y_ij] ∝ norm(y_ij | b0_j, b1, σ2) * norm(0,100) * norm(0,100) * gamma(1,0.01) * gamma (1,0.01)

The joint probability of the parameters given the data is proportional to the probability of these data given the joint probability of the parameters bo_j, b1, and σ2 times the probability of the intercept given the mean and variance times the prior (marginal) probabilities of the mean, the site-level variance, and the overall variance (something like that...)
...
```{r Try it - data}
crabs <- read.csv('C:/Users/piedm/Dropbox/R/GitandR/2021_master/raw-data/8_crab_burrows.csv')
...
crab.mod <- function(){
  ...
  alpha ~ dnorm(0, 0.01) ##check this next; should it be 10?
  b1 ~ dnorm(0, 0.01) ##check this next; should it be 10?
}
```
...
plotPost(exp(fm.crabs.dat$b1), main = 'Effect of salinity on density of crab burrows')
...
##The expected number of burrows is 11.1, with a 95% CI of 9.6 to 12.6.
##The centered intercept becomes the salinity at an average burrow density rather than the salinity at 0 burrows.  If the variable hadn't been centered, it would be the latter.
##For every increase in salinity by one unit, the number of burrows increases by approximately 1.03 burrows (95% CI 1.02 to 1.05)... could that be right?
##If it had no effect then the value 0 should fall within the 95% CI of b1, but it doesn't... I'm wondering if this means that the probability is < 0.05 (and quite a bit less by the look of the histogram).
...
ticks <- read.csv("C:/Users/piedm/Dropbox/R/GitandR/2021_master/raw-data/8_Ehrlichia_occurrence.csv")
glimpse(ticks)
```
...
datList.ticks <- list(
  ...
  site = as.numeric(ticks$site), # site
...
plotPost(fm.ticks.dat$b0, main = "intercept")
plotPost(fm.ticks.dat$b1, main = 'forest')
plotPost(fm.ticks.dat$b2, main = 'edge')
plotPost(fm.ticks.dat$b3, main = 'evergreen')
summary(fm.ticks.dat)
...
*Answer:* My understanding is that the probability of EC presence had a negative relationship with forest cover and amount of edge, but evergreen cover did not have a significant effect on EC presence.  In other words, more edge and more forest cover actually decreased rates of EC presence.  I wonder if that's right...
